...
import com.m2p.elib.util.NetworkUtils
...
fun Context.toast(msg: String) {
    showToast?.apply {
        ...
        Toast.makeText(this.applicationContext, msg, Toast.LENGTH_SHORT).apply {
            showToast = this
        }.show()
    }
}
...
fun Bitmap.saveToAlbum(
    ...
    GlobalScope.launch {
        try {
            ...
            val target =
                File(dirPath, (if (filename.isEmpty()) System.currentTimeMillis().toString() else filename) + ext)
            ...
            MediaScannerConnection.scanFile(
                ELibInit.context, arrayOf(target.absolutePath),
                arrayOf("image/$ext")
            ) { path, uri ->
                runOnUIThread {
